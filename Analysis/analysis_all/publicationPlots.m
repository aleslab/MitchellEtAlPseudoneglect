...
lm2 = scatter(results.plotting.sessions.lm(:,9), results.plotting.sessions.lm(:,4), ...
    'filled', 'o', 'MarkerFaceColor', s2_col); % session1
set(lm2, 'SizeData', 60);
hold on
...
lm4 = scatter(results.plotting.sessions.lm(:,9), results.plotting.sessions.lm(:,6), ...
    'filled', 'o', 'MarkerFaceColor', s4_col); % session4
set(lm4, 'SizeData', 60);
...
%legend boxoff
box off
lmT = title('Landmark'); set(lmT, 'fontsize', 14);
...
ylim([-25 25]);
% Making it prettier
set(ax, 'FontSize', 12, 'ytick', [-20:10:20]);
...
box off
mlbT = title('Line bisection'); set(mlbT, 'fontsize', 14);
%saveas(gcf, pngFileName);
...
box off
trbT = title('Tactile rod bisection'); set(trbT, 'fontsize', 14);
